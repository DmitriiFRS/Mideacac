...
function V8Main({
   ...
   previewTitle: Array<string>;
   previewTitleMobile: Array<string>;
   titleBlock: Array<ImageTitleBlockType>;
   titleBlock2: Array<titleBlock2>;
}) {
   useEffect(() => {
      window.scrollTo(0, 0);
   }, []);
      ...
      <section className={styles.VRFMain}>
         <PreviewTitle>
            <PreviewTitleText
               ...
               previewTitleMobile={previewTitleMobile}
         ...
         {imageTitleParams.map((el, index) => {
            return (
               <Description
                  key={index}
                  bg={titleBlock[index].bg}
                  image={titleBlock[index].image}
                  vector={titleBlock[index].vector}
                  titles={titleBlock[index].titles}
                  index={index}
               />
            );
         })}
