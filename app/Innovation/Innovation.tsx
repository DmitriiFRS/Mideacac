...
import {
  scrollPosition,
  setSidemenuVisible,
} from "../Redux/Slice/mainPageSlice";
...
function Innovation() {
  ...
  const scrollRate = useSelector(
    (state: RootState) => state.mainPageReducer.scrollRate
  );
  const scrollWidth = useSelector(
    (state: RootState) => state.mainPageReducer.scrollWidth
  );
  const isSidebarOpen = useSelector(
    (state: RootState) => state.asideReducer.isSidebarOpen
  );
  useEffect(() => {
  ...
  useEffect(() => {
    function handleScroll() {
      const scrollPercentage =
        ...
        100;
      if (scrollPercentage < 0.2 && scrollPercentage !== null)
        dispatch(scrollPosition(0.2));
  ...
  return (
    ...
    >
      <div
        className={`${styles.innovation__img} ${
          isZoomed ? styles.innovation__img__active : ""
      ...
      <div
        style={{
          filter:
            scrollRate !== null && scrollRate > 28
              ? `blur(${((scrollRate - 27) * 100) / 30}px)`
              : "none",
        }}
        className={`${styles.innovation__img} ${
          !isZoomed ? styles.innovation__img__active : ""
      ...
      <h2
        className={`${styles.innovation__title} ${
          isZoomed ? styles.innovation__title__white : ""
        }
      ${
        (scrollRate !== null && scrollRate > 31) ||
        typeof scrollRate !== "number"
